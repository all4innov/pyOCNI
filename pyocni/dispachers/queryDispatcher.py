...
class QueryDispatcher(object):
    ...
    def get(self):
        ...
        if  self.req.content_type == 'text/occi' or (self.req.body != ""):
            ...
            if jreq is None:
                ...
                var, self.res.status = self.jungler.channel_get_filtered_categories(jreq)

        else:
            #Step[2b]: Retrieve all the categories:
            var, self.res.status = self.jungler.channel_get_all_categories()
    ...
    def post(self):
        ...
        if jBody is None:
            ...
            self.res.body, self.res.status = self.jungler.channel_register_categories(jBody)
    ...
    def put(self):
        ...
        #Step[2]: Update the new data:
            self.res.body, self.res.status = self.jungler.channel_update_categories(jBody)
    ...
    def delete(self):
        ...
        if jBody is None:
            ...
            self.res.body, self.res.status = self.jungler.channel_delete_categories(jBody)
